...
export function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen bg-[#0f1f35]">
      <div className="auth-brand-panel fixed inset-y-0 left-0 hidden w-1/2 flex-col justify-between overflow-hidden px-12 py-10 lg:flex xl:px-16">
        ...
        <Link href="/" className="relative z-10 text-lg font-semibold text-white">
          StudyLens Connect
        </Link>
      ...
      <div className="flex min-h-screen w-full flex-col items-center justify-center px-6 py-12 lg:ml-[50%] lg:w-1/2">
        <Link
          href="/"
          className="mb-10 self-start text-sm text-white/50 transition hover:text-white lg:hidden"
        >
          ← Back to home
        </Link>
